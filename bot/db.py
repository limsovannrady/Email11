...
def upsert_session(telegram_user_id: int, telegram_username: Optional[str],
    ...
    cur.execute("""
        INSERT INTO bot_sessions 
            (telegram_user_id, telegram_username, telegram_first_name, dropmail_session_id, email_address, is_active, last_mail_id, updated_at)
...
def deactivate_session(telegram_user_id: int):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE bot_sessions SET is_active = FALSE, dropmail_session_id = NULL, email_address = NULL, last_mail_id = NULL, updated_at = NOW() WHERE telegram_user_id = %s",
        (telegram_user_id,)
    )
    conn.commit()
    cur.close()
