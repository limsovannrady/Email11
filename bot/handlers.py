"""Reusable Telegram handlers + background jobs.

Imported by both:
  - bot/main.py    — long-polling entry point (local / Replit)
  - api/webhook.py — Vercel serverless webhook
  - api/cron.py    — Vercel cron job runner
"""
...
from telegram import (
    Update, Bot, ReplyKeyboardMarkup, KeyboardButton,
...
POLL_INTERVAL    = 3      # seconds (long-polling mode only)
RESTORE_INTERVAL = 600    # seconds (long-polling mode only)
...
async def handle_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not entries:
        await update.message.reply_text("📭 គ្មាន email ណាទេ។", reply_markup=MAIN_KEYBOARD)
        return

    buttons = [
        [InlineKeyboardButton(f"🗑 {e['email_address']}", callback_data=f"del_email:{e['email_address']}")]
        for e in entries
    ]
    kb = InlineKeyboardMarkup(buttons)
    await update.message.reply_text("ជ្រើសរើស Email ដែលអ្នកចង់លុប៖", reply_markup=kb)
...
# ── Per-entry poll worker (bot-only, no Application context) ─────────────────
async def _poll_one(entry: dict, bot: Bot):
    history_id    = entry["id"]
    ...
    for mail in mails:
        ...
        if TARGET_CHANNEL_ID and TARGET_CHANNEL_ID != user_id:
            try:
                await bot.send_message(chat_id=TARGET_CHANNEL_ID, text=text, parse_mode="HTML")
            except Exception as e:
...
# ── Public API: callable from anywhere (bot context or Vercel cron) ──────────
async def poll_all_emails(bot: Bot):
    entries = await asyncio.to_thread(get_all_history_entries)
    if not entries:
        return 0
    await asyncio.gather(
        *[_poll_one(entry, bot) for entry in entries],
...
# ── job_queue wrappers (long-polling mode) ────────────────────────────────────
async def poll_emails_job(context: ContextTypes.DEFAULT_TYPE):
    await poll_all_emails(context.bot)


async def restore_all_job(context: ContextTypes.DEFAULT_TYPE):
...
def register_handlers(app: Application):
    ...
    app.add_handler(CommandHandler("stats", handle_stats, filters=ALLOWED))
